...
def process_ffmpeg_compose(data, job_id):
    ...
    # Validate output files for image sequences
    if "image2" in [opt["option"] for output in data["outputs"] for opt in output["options"]]:
        matching_files = glob.glob(f"{STORAGE_PATH}/{job_id}_output_0_*.png")
        if not matching_files:
            logger.error(f"No image sequence found for job {job_id}.")
            raise FileNotFoundError(f"No output files found for job {job_id}.")
        output_files = matching_files
